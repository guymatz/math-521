...
Now, notice that (using the WOLG in the first equality and the observation in the inequality)
...
$\forall p \in U \, \exists r > 0 (B_r^M(p) \subseteq U)$.  Since $B_r^N(p) \subseteq U$ and $N \subseteq M$, $B_r^M(p) \subseteq U$, where $B_r^N(p) = B_r^M(p) \cap N$
\\
