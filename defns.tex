...
\begin{definition}{Covering}
	The collection $\mcU$ of subsets of $M$ covers $A \subset M$ if $A$ is contained in the union of the sets belonging to $\mcU$
\end{definition}
...
\begin{definition}{Lebesgue Number}
	A \textbf{Lebesgue Number} for a covering $\mcU$ of $A$ is a positive real number $\lambda$ such that for each $a  \in A$ there is some $U \in \mcU$ with $M_\lambda a \subset U$
\end{definition}
...
\begin{definition}{Totally Bounded}
	...
	\benu
		...
		\item $(\mbR, \text{discrete})$ NOT totally bounded.  Bounded since ball of radius 2 covers, but not totally bounded since it takes $\infty$-ly many 1/2-balls to cover
	\eenu
\end{definition}
...
\subsubsection{7/27}
\begin{theorem}{$2^\omega$ is compact}
	\benu
	\item Either the tree is finite
	or
	\item The tree is infinite, but this leads to a contradiction
	\eenu
\end{theorem}

\begin{theorem}{ $2^\omega \cong$ C}
	
\end{theorem}

\begin{definition}{Dense}
	$S \subseteq M$ is \textbf{dense} in $M$ if cl($S$) = $M$\\
	Examples:
	\benu
	\item $\mbQ$ is dense in $\mbR$
	\item $\mbQ^n$ is dense in $\mbR^n$
	\eenu
\end{definition}

\begin{definition}{Somewhere Dense}
	$S$ is \textbf{Somewhere Dense} in $M$ if there is some open $X \subseteq M$ such that cl($S \cap X) \subseteq X$\\
	Example:\\
	cl(0,1) $\cap \mbQ \cap (0,1) = [0,1] \supseteq (0,1)$\\
	where $S = (0,1) \cap \mbQ$ and $X = (0,1)$ from the definition above
\end{definition}

\begin{definition}{Nowhere Dense}
	$S$ is \textbf{nowhere dense} if it is not \textit{somewhere dense} in $M$
\end{definition}
