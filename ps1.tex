...
\\
It may be the case that when two cuts are "open" cuts, i.e. both irrational, then the union is not $\mbQ$.  For example, the cuts $\sqrt{2}$ and $-\sqrt{2}$.  In this case it is not possible to get $B+B' = 0$?
\\
...
\\
If either $x$ or $x'$ is negative, multiplication get "tricky" and may give incorrect results.
\eenu
...
Let $A|B$ be the cut $s*$.  By 12c we have that $y$ is l.u.b. of $A^*$.  So, for $s^* > 0, (s^*)^n \leq y^n$.  However, we cannot have $y^n < x$ since there are rational numbers as close to x as we like.  We also cannot have $y^n > x$, for if we say $x = y^n - \epsilon$, with $\epsilon > 0$, by 12a there is $s^* \in \mbR^*$ such that $y^n \geq (s^*)^n \geq y^n - \epsilon = n$.  Hence, $y^n = x$
\newpage
\itep 16\\
...
\\
\underline{Induction Hypothesis}: $0 \leq x_k \leq 9$, so $x_k$ is the largest integer less than $10^{k-1}(x-N+ + \frac{x_1}{10^1} + \frac{x_2}{10^2} + \dots + \frac{x_{k-1}}{10^{k-1}})$
\\
\underline{Induction Step}:\
$0 \leq x_{k+1} \leq 9$ so $x_{k+1}$ is the largest integer $\leq 10^k(x - N + \frac{x_1}{10^1} + \frac{x_2}{10^2} + \dots \frac{x_k}{10^k})$
\\
$x_1$ is the largest integer $\leq 10(x - N + \frac{x_1}{10^1} + \frac{x_2}{10^2} + \dots \frac{x_k}{10^k})$. Since $N$ is the largest integer less than $x$, $0 \leq x - N + + \frac{x_1}{10^1} + \frac{x_2}{10^2} + \dots \frac{x_k}{10^k} \leq 1$, and $10(x - N + + \frac{x_1}{10^1} + \frac{x_2}{10^2} + \dots \frac{x_k}{10^k}) < 10$.  $x_1$ is the largest integer less than $10(x -N + + \frac{x_1}{10^1} + \frac{x_2}{10^2} + \dots \frac{x_k}{10^k})$ so $0 \leq x_1 \leq 9$

\item Show that for each $k$ there is an $l \geq k$ such that $x_l \neq 9$
\\
Towards a contradiction, assume that there is no $l \geq k$ such that $x_l \neq 9$.  We would then have the number 9.99999..., However this number is 1.0\hfill\lightning
\\
\item Conversely, show that for each such expression $N.x_1.x_2\dots$ not terminating in an infinite string of 9's, the set
$$ \{N, N + \frac{x_1}{10}, N + \frac{x_1}{10} + \frac{x_2}{100}, \dots\}$$
is bounded and its least upper bound is a real number $x$ with decimal expression $N.x_1.x_2\dots$
\\
\\
The sequence obeys the Cauchy condition, so it has a least upper bound which is the limit of the sequence.
\\
